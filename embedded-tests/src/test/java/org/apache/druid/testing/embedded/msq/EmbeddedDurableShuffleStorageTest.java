  ...
  public void test_selectJoin_sortMerge_durableDestination()
  {
    ...
    final List<S3Object> queryResultObjects =
        storageResource.getS3Client()
                       .listObjects(ListObjectsRequest.builder().bucket(storageResource.getBucket()).prefix(resultsBaseKey)
                           .build())
                       .objectSummaries();
    ...
    Assertions.assertEquals(
        ...
        queryResultObjects.stream().map(S3Object::getKey).collect(Collectors.toSet())
    );
  }
